...
namespace ECSSE
{
    ...
    public:
        ...
        static void parse(std::string str, std::string &op, std::string &ed)
        {
            op = str.substr(0, 1);
            ed = str.substr(1, 8);
        }
        static void parse1(std::string str, std::string &E, std::string &enc)
        {
            E = str.substr(0, 9);
        ...
        {   //server查询
            ...
            std::string _st, l, e, value, enc, E;
            struct timeval t1, t2;
            std::unordered_set<std::string> result_set; //结果集ER
            std::unordered_set<std::string> delete_set; //删除表DT
            _st = st;                                   //状态寄托16字节

            logger::log(logger::INFO) << "服务器开始搜索！"<< uc <<" "<<st<<std::endl;
            for (size_t i = 1; i <= uc; i++)
            {
                ...
                assert(e != "");
                parse1(e, E, enc);  //T[U]= E||enc  9+16才科学
                value = Util::Xor(E, Util::H2(_st)); //E异或H2(st) =(op||ed)
                parse(value, op, ed);
                if (op == "0") //0代表删除
                {
    ...
    {
        std::string server_address("0.0.0.0:50051"); //可修改服务器地址
